-- Keep Lovable Cloud auth signup resilient: auth.users inserts must not depend on
-- application table writes. The user profile is guaranteed after a verified session
-- by public.ensure_current_user_profile().
